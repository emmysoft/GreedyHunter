		...
		for (let c = 1; c <= grid * grid; c++) {
			...
			if (toBeFilled.includes(c)) {
				if (c === toBeFilled[grid]) {
					cell = (
						<div key={c} id={`${c}`} onClick={() => handleClick(c)} className="grid-item">
							{player}
						</div>
					);
				} else {
					cell = (
						<div key={c} id={`${c}`} onClick={() => handleClick(c)} className="grid-item food">
							{food}
						</div>
					);
				}
			} else {
				cell = <div key={c} id={`${c}`} onClick={() => handleClick(c)} className="grid-item"></div>;
			}
			gridArr.push(cell);
		}
